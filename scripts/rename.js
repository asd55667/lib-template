...
const promptForNewName = async (currentInfo) => {
    return new Promise((resolve) => {
        ...
        rl.question('Enter new scope (without @, e.g., "yourscope") or leave empty for no scope: ', (newScope) => {
            rl.question('Enter new package name (e.g., "your-lib"): ', (newPackageName) => {
                ...
                if (!trimmedScope) {
                    ...
                    const scopeWithAt = trimmedScope.startsWith('@') ? trimmedScope : `@${trimmedScope}`;
                    resolve({
...
const updatePackageJsonFiles = async (currentInfo, newInfo) => {
    ...
    try {
        ...
        console.log(err, `Directory ${oldPackageDir} doesn't exist or can't be renamed.`);
    }
...
const updateExamplePackageJson = async (exampleDir, currentInfo, newInfo) => {
    try {
        ...
        console.error(`Error updating package.json in ${exampleDir}: ${err.message}`);
    }
};
...
const processDirectory = async (dir, extensions, currentInfo, newInfo, isUIFile = false) => {
    try {
        const files = await fs.readdir(dir);
        ...
        for (const file of files) {
            ...
            if (stats.isDirectory()) {
                await processDirectory(filePath, extensions, currentInfo, newInfo, isUIFile);
...
const updateImports = async (filePath, currentInfo, newInfo, isUIFile = false) => {
    try {
...
const runPnpmInstall = async () => {
    ...
    try {
        const { stdout, stderr } = await execAsync('pnpm install', { cwd: rootDir });
...
const main = async () => {
    try {
        ...
        console.log(`- Replace all "lib-template" strings with "${newInfo.newPackageName}" (PascalCase "${newInfo.newPascalName}" in UI files)`);
        console.log(`- Update titles in example index.html files`);
        console.log(`- Process markdown files in examples directory (excluding CHANGELOG.md)`);
